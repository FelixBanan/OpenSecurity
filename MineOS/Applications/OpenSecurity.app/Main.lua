...
--------------------------------------------------------------------------------
--Создание cfg файла настроек

local settings = filesystem.readTable(paths.user.applicationData .. "OpenSecurity/settings.cfg")
if settings == false then
	filesystem.writeTable(paths.user.applicationData .. "OpenSecurity/settings.cfg", {redstone = false, doorcontroller = false, rolldoorcontroller = false})
end
...
local function moduleDraw(object)
	...
	screen.drawText(object.x + 3, object.y + 1, textColor, object.module.name)
end
...
for i = 1, #modules do
	...
	if result then
		...
		if success then
			...
			leftPanel.width = math.max(leftPanel.width, unicode.len(result.name) + 6)
		else
